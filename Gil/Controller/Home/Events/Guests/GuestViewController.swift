...
class GuestViewController: KeyboardViewController , SkeletonTableViewDataSource, UITableViewDelegate , UITextFieldDelegate {
    
    var eventId : Int?
    
    let serviceManager = ServiceManager.shared
    
    var friends = [GuestDto]()
    var friendsPendings = [GuestDto]()
    var searchFriends = [GuestDto]()
    ...
    func getFriends(completion: @escaping () -> Void) {
        ...
        serviceManager.getAllGuest(eventId: eventId!){ result in
            DispatchQueue.main.async {
                switch result {
                    ...
                    if let apiError = error as? APIError {
                        switch apiError {
                        case .notFound:
                            self.friends = [GuestDto]()
                            self.searchFriends = [GuestDto]()
                            self.tvFriends.reloadData()
